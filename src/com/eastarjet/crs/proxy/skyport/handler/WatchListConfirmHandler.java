...
public class WatchListConfirmHandler extends ReadLineHandler 
{
	static Logger log = Toolkit.getLogger(WatchListConfirmHandler.class);
	public WatchListConfirmHandler(){ setInputBuffring(true);}
	@Override
	public boolean handleReadLine(int target, Session session,
			Request request, Response response,String line) 
	{
		if(log.isInfoEnabled()) log.info("WatchListConfirmHandler : "+line);
		...
		if("y".equals(line)|| "Y".equals(line))
		{
			...
			if(log.isInfoEnabled()) log.info("WatchListConfirmHandler : cmd="+cmd);
